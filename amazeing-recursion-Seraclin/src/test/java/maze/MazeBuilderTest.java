...
public class MazeBuilderTest {
	...
	@Test(timeout = 50)
	public void testOneWayToOneRoom() {
		String longDesc= "The smell of rancid food permeates the air. There isn't anything of importance.";
		Room room = builder.createRoom(longDesc, "A smelly room.");
		assertFalse(room.isExit());
		assertEquals(longDesc, room.getFullDescription());
		assertEquals("A smelly room.", room.getShortDescription());
		assertTrue(room.getRooms().isEmpty());
		assertEquals(0, room.getRooms().size());
		Room room2 = builder.createRoom("A gray colored room with a fragrant smell", "A smelly room colored gray.");
		Room room3 = builder.createRoom("A bright pink room filled damp with shallow water", "A wet pink room");
		Room room4 = builder.createRoom("A room colored a dull red similar to rust", "A rusty room");
		builder.addOneWayPassage(room2, room);
		builder.addOneWayPassage(room3, room);
		builder.addOneWayPassage(room4, room);
		assertEquals(0, room.getRooms().size());
		assertEquals(1, room2.getRooms().size());
		assertEquals(1, room3.getRooms().size());
		assertEquals(1, room4.getRooms().size());
