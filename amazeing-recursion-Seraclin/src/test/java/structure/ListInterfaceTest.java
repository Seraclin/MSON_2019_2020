...
public class ListInterfaceTest {
	...
	public void testInsertsRemoveAndContains(){
		...
		assertEquals("Size should now be 1", 1, list.size()); // just world
	...
	@Test (timeout = 500)
	public void testOrderFirstAndLast(){
		list.insertLast("Hello").insertLast("World"); // Gecko Hello World Alpha
		assertEquals("Hello is at index 0", 0, list.contains("Hello"));
		assertEquals("World is at index 1", 1, list.contains("World"));
		list.insertFirst("Gecko");
		assertEquals("Gecko", list.getFirst());
		list.insertLast("Alpha");
		assertEquals("Alpha", list.getLast());
	}
	@Test (timeout = 500)
	public void testRemoveInsertAt(){
		list.insertAt(0,"A").insertAt(1,"B");
		assertEquals("A is at index 0", 0, list.contains("A"));
		assertEquals("B is at index 1", 1, list.contains("B"));
		list.insertAt(2,"Gecko");
		assertEquals("Gecko", list.getLast());
		list.insertAt(2,"C");
		assertEquals("Gecko", list.getLast());
		assertEquals("C is at index 2", 2, list.contains("C"));

		list.removeAt(2);
		assertEquals("Gecko is at index 2", "Gecko", list.get(2));
		assertEquals("B is at index 1", "B", list.get(1));
		assertEquals("A is at index 0", "A", list.get(0));

		list.insertAt(2, "C");
		assertEquals("C is at index 2", 2, list.contains("C"));
		list.removeAt(3);
		assertEquals("C is at index 2", 2, list.contains("C"));
		assertEquals("C is at end", "C", list.getLast());

		assertFalse(list.remove("D"));

		assertEquals("Size is 3", 3, list.size());
