...
public class PlusOperatorTest {
  ...
  @Test (timeout = 5000)
  public void testPerformOperationNegativeTwo() {
    operator.setOperand(0, new Operand<Integer>(-4));
    operator.setOperand(1, new Operand<Integer>(-3));

    Operand<Integer> result = operator.performOperation();
    int value = result.getValue();
    assertEquals("Operator applied to -4 and -3 should produce -7.", -7,  value);
  }
  ...
  public void testPerformOperationNegativeOne() {
    ...
    Operand<Integer> result = operator.performOperation();
    int value = result.getValue();
    assertEquals("Operator applied to -4 and 3 should produce -1.", -1,  value);
  }
